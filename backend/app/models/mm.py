...
class ExtShopPrice(Base):       # 조달청 종합쇼핑몰 다수공급자계약(MAS) 실 계약단가
    __tablename__ = "ext_shop_price"
    id: Mapped[int] = mapped_column(BigIntPK, primary_key=True, autoincrement=True)
    spec_name: Mapped[str | None] = mapped_column(String(300))            # prdctSpecNm 품목규격
    corp_name: Mapped[str | None] = mapped_column(String(120))            # cntrctCorpNm 계약기업
    maker_name: Mapped[str | None] = mapped_column(String(120))           # prdctMakrNm 제조사
    contract_price: Mapped[float | None] = mapped_column(Numeric(15, 2))  # cntrctPrceAmt 계약단가
    unit: Mapped[str | None] = mapped_column(String(20))                  # prdctUnit 단위
    contract_method: Mapped[str | None] = mapped_column(String(40))       # cntrctMthdNm 계약방법
    enterprise_div: Mapped[str | None] = mapped_column(String(40))        # entrprsDivNm 기업구분
    delivery_days: Mapped[int | None] = mapped_column(Integer)            # dlvrTmlmtDaynum 납품기한일수
