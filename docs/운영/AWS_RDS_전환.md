...
| 항목 | 단가 | 월 환산(730시간) |
|---|---|---:|
| EC2 t3.small | $0.0260/시간 | $18.98 |
| EC2 t4g.small (ARM, 참고) | $0.0208/시간 | $15.18 |
...
|---|---:|---:|---:|
| RDS 없이 (컨테이너 DB) | $24.45 | $9.35 | $2.44 |
| RDS 분리 (선택한 구성) | **$45.32** | $18.05 | $5.56 |

한화로는 환율 1,345원 기준 24시간 가동 약 60,973원, 하루 8시간 약 24,281원, 면접 때만(월 20시간) 약 7,480원 이다.
...
ARM(t4g.small)으로 바꾸면 `$18.98` → `$15.18` 로 줄지만, arm64 빌드가 없는
패키지에 막히지 않으려고 x86(t3.small)을 유지했다. `instance_type` 한 줄로 바뀐다.
...
| 한 일 | 절감·효과 |
|---|---|
| 인스턴스는 t3.small 유지 | ARM으로 바꾸면 월 $3.80 절약. 호환성을 택했다 |
